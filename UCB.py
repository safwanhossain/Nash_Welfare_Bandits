...
class UCB:
    ...
    def run(self):
        ...
        for t in tqdm(range(self.k, self.T)):
            ...
            parallel_inputs = []
            for sim in range(self.num_sims):
                tup = (sim,  self.last_p[sim], \
                self.empirical_rewards[sim], self.num_samples[sim])
...
def main():
    c, num_sims, T = 0.20, 1, 100
    n, k = 5, 8
    bandit_instance = NSW_Bandit(n, k)
    mu_instance = load_i_instance_nk(n,k,1)
    
    #num_samples= np.ones((k))
    #best_p = solve_cvx(mu_instance, k, n)
    #t=1
    #print("Beforee")
    #print("CVX:", best_p)
    #best_p = solve_torch(mu_instance, num_samples, k, n,t)
    #print("torch:", best_p)

    bandit_instance.set_mu_matrix(mu_instance)

    filename = "UCB"+str(n)+"_k"+str(k)+"_sim"+\
